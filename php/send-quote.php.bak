...
// Handle file upload
$file_link_html = '<p style="color: #999;">No file attached</p>';
$file_info_for_customer = '';
...
if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
    ...
    // Also check by extension for AI/EPS files
    ...
    if ((in_array($file['type'], $allowed_types) || in_array($file_extension, $allowed_extensions)) 
        ...
        if (move_uploaded_file($file['tmp_name'], $filepath)) {
            ...
            ";
            $file_info_for_customer = $file['name'];
        }
    }
}
...
if (!empty($file_info_for_customer)) {
    $file_attachment_info = "
        <div style='background: #edf2f7; padding: 10px 15px; border-radius: 5px; margin: 15px 0;'>
            <strong>📎 Attached file:</strong> " . htmlspecialchars($file_info_for_customer) . "
        </div>
    ";
}
